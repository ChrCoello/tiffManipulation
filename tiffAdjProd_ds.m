function tiffAdjProd_ds(input,output_dir,varargin)
%
...
if ~isempty(filenames)
    ...
    for idxFile = 1 : nFiles
        ...
        if p.Results.make_thumb
            thumbData = imresize(dataRaw,[1024 1024*tiffInfo.Width./tiffInfo.Height]);
        ...
        % Adjust intensity per tile using thumb
        fprintf(1,' -- Adjusting histogram ...\n');
        tic;
        sat_pix = p.Results.sat_pix;
        tmp_im = imresize(dataRaw,[2048 2048*tiffInfo.Width./tiffInfo.Height]);
        [prcMin, prcMax] = adjustIntensityBW(tmp_im,sat_pix);
        adjData = imadjust(dataRaw,[prcMin; prcMax],[0 0 0;1 1 1]);
        clear tmp_im dataRaw
        tileTime=toc;
...
%         end
        ...
        if p.Results.make_thumb
            %
            thumbData = imresize(adjData,[1024 1024*tiffInfo.Width./tiffInfo.Height]);
...
function [prcMin, prcMax] = adjustIntensityBW(slicedata,sat_pix)
% Description needed
%
